...
val_loader = DataLoader(val_dataset, batch_size=num_test_sample,
...
val_P = val_P[:num_test_sample, ...]
val_H = val_H[:num_test_sample, ...]
...
nmse_all = np.zeros((len(lmbda_range), len(lr_range),
                     len(snr_range), num_test_sample))  # Should match data
complete_log = np.zeros((len(lmbda_range), len(lr_range),
                         len(snr_range), gd_iter, num_test_sample))
result_dir = os.path.join(PROJECT_DIR, f'results/l1CS_lifted{lifting}/train-{args.train}_test-{args.test}')
...
for snr_idx, local_snr in enumerate(snr_range):
    ...
    print(f'SNR = {local_snr} dB, NMSE = {10 * np.log10(best_nmse[snr_idx])} dB using lambda = { best_lmbda[snr_idx]} and step size = {best_lr[snr_idx]}')
...
plt.grid()
plt.legend()
plt.title(f'Channel estimation, lifting = {args.lifting}')
plt.xlabel('SNR [dB]')
...
plt.figure(figsize=(10, 10))
plt.plot(snr_range, best_nmse / config.data.image_size[0] / config.data.image_size[1], linewidth=4, label=f'{args.test}, Lasso')
plt.grid()
plt.legend()
plt.title(f'Channel estimation, lifting = {args.lifting}')
plt.xlabel('SNR [dB]')
