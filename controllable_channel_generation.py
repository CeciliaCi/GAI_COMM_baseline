...
# -*- coding: utf-8 -*-
"""
The conditioned channel generation by classifier-guided conditional DM.
Predictor-corrector sampling has been implemented here for MIMO channel generation.
"""
...
def conditional_predictor_update_fn(X_coefficients, t, Y, Pilot, channel_noise, model, sde, predictor, continuous, probability_flow):
  """
  The predictor update function for classifier-guided sampling.
  @:param
      X_coefficients:  real/image parts of H, [batch_size, 2, n_tx, n_rx]
      Y:  received measurement of pilot signals
      P:  pilot signal
  @:return A function for predictor update
  """
...
def get_pc_channel_sampler(sde, shape, predictor, corrector, snr,
  ...
  @:return A classifier-guided image sampler.
  """
  ...
  def pc_channel_sampler(model, Y, Pilot, channel_noise, ground_truth):
    """Generate classifier-guided samplers with Predictor-Corrector (PC) method.
    @:param
      model: A score model.
      Y: Measurement matrix of each sample.
      Pilot: Pilot signal.
      channel_noise: noise power
      ground_truth: ground_truth samples.
    @:return
